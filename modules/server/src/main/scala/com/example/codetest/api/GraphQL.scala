...
trait GraphQL[F[_]] {
  ...
  /** Executes a JSON-encoded request in the standard POST encoding, described thus in the spec:
    *
    * A standard GraphQL POST request should use the application/json content type, and include a JSON-encoded body of
    * the following form:
    *
    * { "query": "...", "operationName": "...", "variables": { "myVariable": "someValue", ... } }
    *
    * `operationName` and `variables` are optional fields. `operationName` is only required if multiple operations are
